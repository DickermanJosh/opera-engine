...
SearchResult SearchEngine::search(const SearchLimits& limits) {
    ...
    alphabeta->stop_check = [this]() {
        ...
        if (progress && should_output_info()) {
            current_info.nodes = nodes_searched + alphabeta->get_stats().nodes;
            current_info.time_ms = get_elapsed_time_ms();
            current_info.nps = current_info.nodes * 1000 / std::max<uint64_t>(1, current_info.time_ms);
            progress(current_info);
            last_info_time = std::chrono::steady_clock::now();
        }
        if (!current_limits.infinite &&
            (nodes_searched + alphabeta->get_stats().nodes >= current_limits.max_nodes ||
             get_elapsed_time_ms() >= current_limits.max_time_ms)) {
...
SearchResult SearchEngine::iterative_deepening() {
    ...
    generateAllLegalMoves(board, legal_moves, board.getSideToMove());
    ...
    if (!alphabeta->root_moves.empty()) {
        bool found = false;
        for (size_t i = 0; i < legal_moves.size(); ++i) {
            const auto& mg = legal_moves[i];
            Move candidate(mg.from(), mg.to(), mg.isPromotion() ? PROMOTION : NORMAL,
                mg.isPromotion() ? typeOf(mg.promotionPiece()) : NO_PIECE_TYPE);
            if (std::find(alphabeta->root_moves.begin(), alphabeta->root_moves.end(), candidate.toString()) != alphabeta->root_moves.end()) {
                best_result.best_move = candidate; found = true; break;
            }
        }
        if (!found) { best_result.best_move = NULL_MOVE; return best_result; }
    }
...
void SearchEngine::update_search_info(int depth, int score, uint64_t nodes) {
    ...
    if (progress) progress(current_info);
...
void SearchEngine::set_hash_size(size_t mb) {
    // Recreate dependent objects before destroying the table they reference.
    alphabeta.reset(); move_ordering.reset();
    tt = std::make_unique<TranspositionTable>(mb);
    move_ordering = std::make_unique<MoveOrdering>(board, *tt);
    alphabeta = std::make_unique<AlphaBetaSearch>(board, stop_flag, *tt, *move_ordering, *see,
        use_morphy_style ? static_cast<eval::Evaluator*>(morphy_evaluator.get()) : evaluator.get());
}
} // namespace opera
