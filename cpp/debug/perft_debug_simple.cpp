/**
 * Simple Perft Debug Tool - Detailed move analysis for failing tests
 */
...
// Forward declaration
uint64_t perftRecursive(Board& board, int depth);
...
void perftDivide(Board& board, int depth) {
    ...
    for (size_t i = 0; i < moves.size(); ++i) {
        ...
        uint64_t nodes = (depth > 1) ? perftRecursive(tempBoard, depth - 1) : 1;
        totalNodes += nodes;

        std::cout << std::setw(6) << move.toString() << ": " << std::setw(12) << nodes;
...
uint64_t perftRecursive(Board& board, int depth) {
    ...
    for (size_t i = 0; i < moves.size(); ++i) {
        ...
        if (tempBoard.makeMove(moves[i])) {
            nodes += perftRecursive(tempBoard, depth - 1);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    int testIndex = 0;
    int depth = 1; // Start with depth 1 for quick analysis
    ...
    std::cout << "Expected at depth " << test.depth << ": " << test.expected << " nodes\n";
