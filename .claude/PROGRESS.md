...
The release-candidate review adds correct negamax score perspective, bounded shutdown when stdout is unread, and safe logging initialization. Process acceptance passes 21/21 on macOS and Docker/Linux ARM64; an independent Stockfish oracle validates 176 positions on each. The macOS C++ bridge passes both harnesses under ASan/UBSan. Rust library/search/protocol checks pass 281/281; focused C++ checks pass 40/40. Broad results are 342/345 Rust and 433/454 C++. The external python-chess client passes lifecycle checks and complete timed Stockfish games on macOS/Linux. The candidate is pushed, but GitHub Actions is disabled; approval to enable it and Windows verification are pending. `uci` has not been merged. See [dated evidence](../docs/current-baseline.md) and the [core → neural evaluation → Unity roadmap](../docs/development-roadmap.md).
