...
use crate::uci::{ChessMove, UCICommand};
...
impl PositionCommandHandler {
    ...
    pub fn new() -> UCIResult<Self> {
        let board = Board::new()
            .with_context(ErrorContext::new(
                "Failed to create board for position handler",
            ))
            .map_err(|e| e.error)?;
    ...
    pub fn handle_position_command<'a>(&mut self, cmd: &UCICommand<'a>) -> UCIResult<()> {
        match cmd {
            UCICommand::Position { position, moves } => {
                ...
                info!("Position set successfully: {} moves applied", moves.len());
        ...
        match position {
            ...
            crate::uci::Position::Fen(fen_string) => {
                ...
                self.board
                    ...
                    .with_context(
                        ErrorContext::new("Invalid FEN string").detail(fen_string.to_string()),
                    )
                    .map_err(|e| e.error)?;
    ...
    fn apply_move_sequence<'a>(&mut self, moves: &[ChessMove<'a>]) -> UCIResult<()> {
        ...
        for (index, chess_move) in moves.iter().enumerate() {
            ...
            let is_valid = self
                .board
                .is_valid_move(&move_str)
                .with_context(ErrorContext::new("Failed to validate move").detail(format!(
                    "move: {}, index: {}",
                    move_str,
                    index + 1
                )))
            ...
            if !is_valid {
                return Err(UCIError::Move {
                    message: format!(
                        "Invalid move '{}' at position {} in sequence",
                        move_str,
                        index + 1
                    ),
                });
            ...
            self.board
                .make_move(&move_str)
                .with_context(ErrorContext::new("Failed to apply move").detail(format!(
                    "move: {}, index: {}",
                    move_str,
                    index + 1
                )))
    ...
    pub fn is_checkmate(&self) -> UCIResult<bool> {
        self.board
            .is_checkmate()
            .with_context(ErrorContext::new(
                "Failed to check if position is checkmate",
            ))
            .map_err(|e| e.error)
    }
    ...
    pub fn is_stalemate(&self) -> UCIResult<bool> {
        self.board
            .is_stalemate()
            .with_context(ErrorContext::new(
                "Failed to check if position is stalemate",
            ))
            .map_err(|e| e.error)
    }
...
mod tests {
    use super::*;
    use crate::uci::{ChessMove, Position};
    ...
    fn test_create_position_handler() {
        ...
        assert!(
            handler.is_ok(),
            "Should create position handler successfully"
        );

        let handler = handler.unwrap();
        assert_eq!(
            handler.get_move_history().len(),
            0,
            "Should start with empty move history"
        );
    }
    ...
    fn test_setup_starting_position() {
        ...
        assert!(
            result.is_ok(),
            "Should handle startpos command successfully"
        );
    ...
    fn test_setup_fen_position() {
        ...
        assert!(
            result.is_ok(),
            "Should handle FEN position command successfully"
        );
    ...
    fn test_apply_moves_from_startpos() {
        ...
        assert!(
            result.is_ok(),
            "Should apply moves from starting position successfully"
        );

        assert_eq!(
            handler.get_move_history().len(),
            2,
            "Should record 2 moves in history"
        );
        assert_eq!(
            handler.get_move_history()[0],
            "e2e4",
            "Should record first move"
        );
        assert_eq!(
            handler.get_move_history()[1],
            "e7e5",
            "Should record second move"
        );
    }
    ...
    fn test_move_validation() {
        ...
        assert!(
            handler.validate_move("e2e4").unwrap(),
            "e2e4 should be valid"
        );
        assert!(
        ...
        assert!(
            handler.validate_move("e2e4x").is_err(),
            "Malformed move should error"
        );
        assert!(
            handler.validate_move("z1a1").is_err(),
            "Invalid square should error"
        );
    }
    ...
    fn test_promotion_moves() {
        ...
        assert_eq!(
            history[0], "a7a8q",
            "Should record promotion move correctly"
        );
    }
}
