...
use crate::{uci_options, AUTHOR, NAME, VERSION};
...
impl BasicCommandHandler {
    ...
    pub fn handle_uci_command(&self) -> UCIResult<Vec<String>> {
            ...
            let response = match *option_type {
                ...
                _ => {
                    return Err(UCIError::Configuration {
                        message: format!(
                            "Unknown option type '{}' for option '{}'",
                            option_type, name
                        ),
                    });
                }
        ...
        debug!(
            "Generated {} UCI option responses",
            uci_options::OPTIONS.len()
        );
    ...
    pub fn handle_isready_command(&self) -> UCIResult<Vec<String>> {
        ...
        let can_accept_commands =
            matches!(current_state, EngineState::Ready | EngineState::Searching);
...
mod tests {
    ...
    fn create_test_handler() -> BasicCommandHandler {
        ...
        state
            .transition_to(EngineState::Ready, "Test initialization")
    ...
    fn test_uci_command_response() {
        ...
        let option_responses: Vec<_> = responses
            .iter()
        ...
        assert!(responses
            .iter()
            .any(|r| r.contains("option name MorphyStyle")));
    }
    ...
    fn test_spin_option_ranges() {
        ...
        assert_eq!(
            handler.get_spin_option_range("SacrificeThreshold"),
            (0, 500)
        );
    ...
    fn test_all_uci_options_covered() {
        ...
        for (option_name, _, _) in uci_options::OPTIONS {
            assert!(
                responses
                    .iter()
                    .any(|r| r.contains(&format!("option name {}", option_name))),
    ...
    fn test_uci_response_order() {
        ...
        for i in option_start..option_end {
            assert!(
                responses[i].starts_with("option"),
                "Response at index {} should be an option: {}",
                i,
                responses[i]
            );
        }
