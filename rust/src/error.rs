...
pub mod never_panic {
    ...
    pub fn safe_parse<T: std::str::FromStr>(
        input: &str,
        operation: &str,
    ) -> UCIResult<T> {
    ...
    pub fn safe_get<T: Clone>(
        ...
        collection.get(index).cloned().ok_or_else(move || UCIError::Internal {
            message: format!("Index {} out of bounds in {} (length: {})", 
                           index, context, collection.len()),
        })
    }
    ...
    pub fn safe_slice(
        ...
        if start > len {
            return Err(UCIError::Protocol {
                message: format!("Start index {} exceeds string length {} in {}", 
                               start, len, context),
            });
        }
        ...
        if end > len {
            return Err(UCIError::Protocol {
                message: format!("End index {} exceeds string length {} in {}", 
                               end, len, context),
            });
        }
        ...
        if start > end {
            return Err(UCIError::Protocol {
                message: format!("Start index {} > end index {} in {}", 
                               start, end, context),
            });
        }
