...
pub mod assertions {
    ...
    pub fn assert_valid_uci_response(response: &str) -> UCIResult<()> {
        ...
        if !valid_prefixes.iter().any(|prefix| response.starts_with(prefix)) {
            return Err(UCIError::Protocol {
...
macro_rules! benchmark_test {
    ($name:ident, $max_duration:expr, $body:block) => {
        #[tokio::test]
        async fn $name() -> $crate::error::UCIResult<()> {
            use $crate::testing::{TestRuntime, TestConfig, PerformanceMeasurer};
            ...
            runtime.run_test(stringify!($name), async move {
                let result = measurer.measure(stringify!($name), async {
                    $body
                }).await;
...
mod tests {
    ...
    async fn test_performance_measurer() {
        ...
        let result = measurer.measure("test_op", async {
    ...
    async fn test_assertions() -> UCIResult<()> {
        ...
        let error = UCIError::Protocol { message: "test".to_string() };
