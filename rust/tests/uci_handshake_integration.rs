...
use opera_uci::{
    BasicCommandHandler, EngineIdentification, EngineState, ResponseFormatter, UCIEngine,
    UCIResponse,
};
...
async fn test_complete_uci_handshake() {
    ...
    for _ in 0..20 {
        // Allow up to 20 responses (id + options + uciok)
    ...
    assert!(
        !uci_responses.is_empty(),
        "UCI command should generate responses"
    );
    assert!(uciok_received, "UCI command should end with 'uciok'");

    // Should have id name and id author at the beginning
    assert!(
        uci_responses.iter().any(|r| r.starts_with("id name")),
        "Should have id name response"
    );
    assert!(
        uci_responses.iter().any(|r| r.starts_with("id author")),
        "Should have id author response"
    );

    // Should have option declarations
    let option_responses: Vec<_> = uci_responses
        .iter()
        .filter(|r| r.starts_with("option"))
        .collect();
    assert!(
        !option_responses.is_empty(),
        "Should have at least one option declared"
    );

    // Should end with uciok
    assert_eq!(
        uci_responses.last().unwrap(),
        "uciok",
        "Should end with uciok"
    );
...
async fn test_uci_options_declaration() {
    ...
    let option_responses: Vec<_> = all_responses
        .iter()
        .filter(|r| r.starts_with("option"))
        .collect();
    ...
    assert!(
        option_responses.iter().any(|r| r.contains("name Hash")),
        "Should have Hash option"
    );

    // Should have Threads option
    assert!(
        option_responses.iter().any(|r| r.contains("name Threads")),
        "Should have Threads option"
    );

    // Validate option format (at least one should be properly formatted)
    let hash_option = option_responses
        .iter()
        .find(|r| r.contains("name Hash"))
        .expect("Hash option should exist");
    assert!(
        hash_option.contains("type spin"),
        "Hash should be spin type"
    );
...
async fn test_isready_various_states() {
    ...
    engine
        .process_command("isready")
        .await
        .expect("IsReady should succeed in Ready state");
...
async fn test_quit_command() {
    ...
    timeout(Duration::from_millis(100), state_changes.recv())
        .await
        .ok();
...
async fn test_basic_command_handler_integration() {
    ...
    state
        .transition_to(opera_uci::EngineState::Ready, "Test setup")
        .expect("Should set ready state");

    let handler = BasicCommandHandler::new(id_info, state);

    // Test UCI command
    let uci_responses = handler
        .handle_uci_command()
        .expect("UCI command should succeed");

    assert!(!uci_responses.is_empty(), "UCI should generate responses");
    assert!(
        uci_responses.iter().any(|r| r.contains("id name")),
        "Should have id name"
    );
    assert!(
        uci_responses.iter().any(|r| r.contains("id author")),
        "Should have id author"
    );
    assert_eq!(
        uci_responses.last().unwrap(),
        "uciok",
...
fn test_response_formatting_integration() {
    ...
    let formatted =
        ResponseFormatter::format_batch(&responses).expect("Should format responses successfully");
...
async fn test_full_uci_session() {
    ...
    engine
        .process_command("uci")
        .await
        .expect("UCI command should work");
    ...
    engine
        .process_command("isready")
        .await
        .expect("IsReady should work after options");
...
async fn test_basic_command_performance() {
    ...
    assert!(
        ...
        "100 isready commands should complete in under 100ms, took {:?}",
        elapsed
    );
    ...
    assert!(
        ...
        "10 UCI commands should complete quickly, took {:?}",
        elapsed2
    );
}
