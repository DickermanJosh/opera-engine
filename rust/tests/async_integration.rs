...
use opera_uci::testing::{TestRuntime, TestConfig, MockUCICommands, PerformanceMeasurer};
...
async fn test_basic_async_runtime() -> UCIResult<()> {
    ...
    runtime.run_test("basic_async_op", async {
...
async fn test_timeout_handling() -> UCIResult<()> {
    ...
    // This should timeout
    let result = runtime.run_test("timeout_test", async {
...
async fn test_concurrent_operations() -> UCIResult<()> {
    ...
    runtime.run_test("concurrent_test", async {
        let tasks = vec![
...
async fn test_performance_measurement() -> UCIResult<()> {
    ...
    runtime.run_test("performance_test", async {
        let mut measurer = PerformanceMeasurer::new();
        
        // Measure several operations
        let _result1 = measurer.measure("op1", async {
            tokio::time::sleep(Duration::from_millis(5)).await;
            42
        }).await;
        
        let _result2 = measurer.measure("op2", async {
            tokio::time::sleep(Duration::from_millis(10)).await;
            "test"
        }).await;
        
        let _result3 = measurer.measure("op1", async {
...
async fn test_mock_uci_commands() -> UCIResult<()> {
    ...
    runtime.run_test("mock_commands_test", async {
...
async fn test_async_error_propagation() -> UCIResult<()> {
    ...
    let result = runtime.run_test("error_propagation", async {
...
async fn test_async_channel_communication() -> UCIResult<()> {
    ...
    runtime.run_test("channel_communication", async {
...
async fn benchmark_async_operations() -> UCIResult<()> {
    ...
    runtime.run_test("async_benchmark", async {
        // Benchmark different async operations
        let _result = measurer.measure("sleep_10ms", async {
            tokio::time::sleep(Duration::from_millis(10)).await;
        }).await;
        
        let _result = measurer.measure("concurrent_sleeps", async {
            let tasks = vec![
        ...
        let _result = measurer.measure("channel_throughput", async {
        ...
        let sleep_time = measurer.get_measurement("sleep_10ms")
        ...
        let concurrent_time = measurer.get_measurement("concurrent_sleeps")
            .ok_or_else(|| UCIError::Internal {
