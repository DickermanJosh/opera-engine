...
use opera_uci::{ChessMove, Position, PositionCommandHandler, UCICommand};
...
fn test_complete_position_workflow() {
    ...
    handler
        .handle_position_command(&startpos_cmd)
        .expect("Should handle startpos");
    ...
    handler
        .handle_position_command(&moves_cmd)
        .expect("Should apply moves");
    ...
    let reset_fen = handler
        .get_current_position()
        .expect("Should get reset FEN");
    assert_eq!(
        reset_fen,
        "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
    );
...
fn test_fen_position_handling() {
    ...
    handler
        .handle_position_command(&fen_cmd)
        .expect("Should handle FEN position");

    let current_fen = handler
        .get_current_position()
        .expect("Should get current FEN");
    ...
    assert_eq!(
        reset_fen, test_fen,
        "Should reset to stored FEN, not starting position"
    );
}
...
fn test_error_handling() {
    ...
    let fen = handler
        .get_current_position()
        .expect("Should get FEN after error");
    assert_eq!(
...
fn test_board_access() {
    ...
    let fen = board
        .get_fen()
        .expect("Should get FEN through board reference");
    assert_eq!(
        fen,
        "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
    );
...
fn test_move_sequence_from_fen() {
    ...
    handler
        .handle_position_command(&cmd)
        .expect("Should handle FEN + moves");
    ...
    let final_fen = handler
        .get_current_position()
        .expect("Should get final FEN");
    assert_ne!(
        final_fen, test_fen,
